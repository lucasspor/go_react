  ...
  return (
    <li data-answered={answered} className="ml-4 leading-relaxed text-zinc-100 data-[answered=true]:opacity-50 data-[answered=true]:pointer-events-none">
     {text}
      ...
      {hasReacted ? (
        <button  type="button" className="flex items-center text-orange-400 hover:text-orange-500 gap-2 mt-3">
          <ArrowUp className='size-4' />
          Curtir pergunta ({amountOfReactions})
        </button>
      ) : (
        <button onClick={handleReactToMessage} type="button" className="flex items-center text-zinc-400 gap-2 mt-3 hover: text-zinc-300">
          <ArrowUp className='size-4' />
          Curtir pergunta ({amountOfReactions})
        </button>
      )
